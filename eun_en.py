...
class PkgIconWindow(Gtk.ApplicationWindow):
    def __init__(self,application):
        super().__init__(application=application,title="Easy Uninstall")
        self.set_default_size(800, 600)
        self.set_border_width(10)

        # 图标映射
        self.statusbar = Gtk.Statusbar()
        self.statusbar.push(0, "Scanning desktop icons")
        ...
        self.statusbar.push(0, f"Ready，total {len(self.pkg_icons)} software pakages have icon")
        ...
        self.show_all_btn = Gtk.CheckButton(label="Show all package")
        ...
        col_name = Gtk.TreeViewColumn("Package", renderer_text, text=1)
        col_name.set_resizable(True)
        col_name.set_min_width(150)
        self.treeview.append_column(col_name)

        # 版本列
        col_ver = Gtk.TreeViewColumn("Version", Gtk.CellRendererText(), text=2)
        col_ver.set_resizable(True)
        self.treeview.append_column(col_ver)

        # 描述列
        col_desc = Gtk.TreeViewColumn("Discription", Gtk.CellRendererText(), text=3)
        ...
        item_uninstall = Gtk.MenuItem(label="Uninstall this package")
        ...
        # “清除多余软件包”按钮，放在右边
        self.autoremove_btn = Gtk.Button(label="Clean useless files")
    ...
    def populate_model(self):
        ...
        for pkg in cache:
            if pkg.installed:
                ...
                count += 1
        self.statusbar.push(0, f" {count} packages have been loaded")
    ...
    def on_uninstall(self, menuitem):
        ...
        dialog = Gtk.MessageDialog(
            ...
            text=f"Are you sure you want to uninstall “{pkg_name}” ？"
        )
        dialog.format_secondary_text("Use ‘apt purge’ to completely remove this package")
        response = dialog.run()
        dialog.destroy()
        ...
        self.statusbar.push(0, f"Uninstalling {pkg_name} ...")
    ...
    def _show_uninstall_wait_dialog(self, pkg_name):
        ...
        spinner = Gtk.Spinner()
        spinner.start()
        label = Gtk.Label(label=f"Uninstalling {pkg_name}")
    ...
    def _uninstall_package(self, pkg_name, child_iter):
        ...
        try:
            ...
            success = False
            error_msg = f"Failed to uninstall：{e.stderr or e}"
    ...
    def _on_uninstall_finished(self, pkg_name, success, error_msg, child_iter):
        ...
        if success:
            ...
            self.statusbar.push(0, f"{pkg_name} was successfully uninstalled")
        else:
    ...
    def on_autoremove_clicked(self, button):
        ...
        dialog = Gtk.MessageDialog(
            ...
            message_type=Gtk.MessageType.QUESTION,
            buttons=Gtk.ButtonsType.OK_CANCEL,
            text="Are you sure you want to clean all useless files？"
        )
        dialog.format_secondary_text("Use ‘apt autoremove’ to clean all useless files")
        response = dialog.run()
        dialog.destroy()
        ...
        self.statusbar.push(0, "Cleaning useless package")
    ...
    def _show_autoremove_wait_dialog(self):
        ...
        spinner = Gtk.Spinner()
        spinner.start()
        label = Gtk.Label(label="Cleaning useless files")
        label.set_margin_start(10)
        box.pack_start(spinner, False, False, 10)
        box.pack_start(label, False, False, 10)
    ...
    def _run_autoremove(self):
        ...
        try:
            ...
            success = False
            error_msg = f"Failed to clean：{e.stderr or e}"
        except Exception as e:
            success = False
            error_msg = f"Error：{e}"
    ...
    def _on_autoremove_finished(self, success, error_msg):
        ...
        if success:
            ...
            self.filter_model.refilter()
            self.statusbar.push(0, "Useless files cleanup complete,list refreshed")
        else:
            self.statusbar.push(0, error_msg or "Unknow error")
#--桌面图标修补后半部--
#---------程序入口--------------
def on_activate(app):
